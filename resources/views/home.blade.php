...
<head>
    ...
    <title>Inicio {{ Auth::user()->name }} </title>    
</head>
...
@include('layouts.header',[])
    <div class="container">
        ...
        <br>@foreach( $publicaciones as $publicacion)
        <div id="post" class="card">
            <span style="font-size: 16px;">  
                <?php
                    ...
                    if($pd= True and $pub_user== Auth::user()->id) {                 
                        $pub_Del = $pd['id']; 
                ?>                           
                    <form action="{{ url('/post/'.$publicacion->id ) }}" method="post">
                        @csrf
                        {{ method_field('DELETE') }}
                ...
                <img src="{{ asset('storage').'/'.$publicacion->foto_user}}" id="perfil">
                    ...
                    <img src="{{ asset('storage').'/'.$publicacion->imagen}}" class="foto" width="500px" height="auto" alt="...">
                    <div class="card-body">
                    <!-- LIKES -->    
                    <?php
                        $db= mysqli_connect('localhost','root','','social_network');
                ...
                <a data-toggle="tooltip" style="margin-top: 0px"> <h style="font-size: 16px;" class="card-text" >{{$publicacion->contenido}}</h>  
                    <?php
                        $db= mysqli_connect('localhost','root','','social_network');
                        ...
                        if($pd= True and $pub_user== Auth::user()->id){   
                        ?>
                        | <a href="{{ url('/post/'.$publicacion->id.'/edit') }}" class="btn" style="margin-left: -10px;  font-style: italic;"> Editar post</a>
                        <?php
                            }
            ...
            ?>
                    <a href="{{ url('/comentarios/'.$id_comen.'/edit') }}" class="btn">
